...
decimal, not binary.)
"""
from binary_diagnostic import gamma_epsilon

DIGITS = 12


def oxygen_co2(numbers):
    # Initialize oxygen and scrubber.
    oxygen = numbers.copy()
    scrubber = numbers.copy()
    final_oxygen = None
    final_scrubber = None
    # Begin eliminating numbers:
    for i in range(DIGITS):
        if not final_oxygen:
            oxygen_digit = gamma_epsilon(oxygen)[0]
            oxygen_digit = (oxygen_digit >> (DIGITS - i - 1)) & 1
            oxygen = [x for x in oxygen if (x >> (DIGITS - i - 1)) & 1 == oxygen_digit]
        if not final_scrubber:
            scrubber_digit = gamma_epsilon(scrubber)[1]
            scrubber_digit = (scrubber_digit >> (DIGITS - i - 1)) & 1
            scrubber = [x for x in scrubber if (x >> (DIGITS - i - 1)) & 1 == scrubber_digit]
        if len(oxygen) == 1:
            final_oxygen = oxygen[0]
        if len(scrubber) == 1:
            final_scrubber = scrubber[0]
    return (final_oxygen, final_scrubber)
